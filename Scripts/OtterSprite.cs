...
public partial class OtterSprite : Sprite3D
{
    ...
    private record ChunkData(RigidBody3D Chunk, Vector3 TargetPosition, Vector3 TargetScale);
    ...
    private static Color GetChunkColor(Image textureImage, int x, int y, int chunkSizeX, int chunkSizeY, Vector2I textureSize)
    {
...
// Rollback logic using Tweens, and awaiting the first Tween's completion
    private async Task RollbackChunks(float duration)
    {
        ...
        foreach (var (chunk, targetPosition, _) in _activeChunks)
        {
            ...
            tweenFade.TweenProperty(chunk, "global_position", targetPosition+ new Vector3(10,-10,0), fadeDuration) // Use data.TargetPosition
                .SetEase(Tween.EaseType.InOut);
        }
        
        tweenFade.TweenProperty(this, "modulate", Colors.White, fadeDuration);
