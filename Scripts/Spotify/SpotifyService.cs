...
public class SpotifyService
{
    ...
    public SpotifyService(AccessTokenService accessTokenService) 
    {
        ...
        if (!string.IsNullOrEmpty(accessTokenService.GetSpotifyAccessToken()))
        {
            _spotify ??= new SpotifyClient(SpotifyClientConfig
                ...
                .WithAuthenticator(new AuthorizationCodeAuthenticator(accessTokenService.GetClientId(),accessTokenService.GetClientSecret(), new AuthorizationCodeTokenResponse()
                {
        ...
        catch (Exception)
        {
            ...
            catch (Exception)
            {
                ...
                _spotify = new SpotifyClient(SpotifyClientConfig
                    .CreateDefault()
                    .WithAuthenticator(
                        new AuthorizationCodeAuthenticator(accessTokenService.GetClientId(), accessTokenService.GetClientSecret(), response)));
            }
        }
    ...
    private static async Task<string> GetCurrentlyPlayingAsync()
    {
    ...
    /// <returns></returns>
    /// <exception cref="SongNotFoundException"></exception>
    private static async Task<string> AddSongToQueue(string name)
    {
