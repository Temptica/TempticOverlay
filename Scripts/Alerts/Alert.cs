...
public abstract class Alert
{
	...
	public void ProcessAlert(double delta)
	{
		...
		if (ElapsedTime >= Duration && !DisplayServer.TtsIsSpeaking() && !DisplayServer.TtsIsPaused())
		{
			IsFinished = true;
			StopAlert();
			return;
		...
		{
			TtsStarted = true;
			DisplayServer.TtsSpeak(TTSMessage, DisplayServer.TtsGetVoicesForLanguage("en")[0]);
		}
	...
	protected virtual void StartAlert()
	{
		switch (Event)
		{
			...
			case AlertType.Bit:
				...
				break;
			case AlertType.Raid when User.Equals("iheartfunnyboys", StringComparison.CurrentCultureIgnoreCase):
				AudioPlayer.PlayAudio(AudioEffects.IHeartRaid);
				break;
			case AlertType.Raid:
